...
async function getAttributes(id) {
  ...
  const response = await fetch(
    `https://api-mainnet.magiceden.dev/v3/rtp/ethereum/tokens/v6?collection=0x34d85c9cdeb23fa97cb08333b511ac86e1c4e258&tokenName=${id}&sortBy=floorAskPrice&limit=1&includeTopBid=false&excludeEOA=false&includeAttributes=True&includeQuantity=false&includeDynamicPricing=false&includeLastSale=false&normalizeRoyalties=false`,
    options
  );
...
async function getInfoEarth(id) {
  ...
  const infoEarth = {
    ...
    usdPrice: dataEarth.market.floorAsk &&
      dataEarth.market.floorAsk.price &&
      dataEarth.market.floorAsk.price.amount ?
      dataEarth.market.floorAsk.price.amount.usd.toFixed(2) : "N/A",
    ethPrice: dataEarth.market.floorAsk &&
      dataEarth.market.floorAsk.price &&
      dataEarth.market.floorAsk.price.amount ?
      dataEarth.market.floorAsk.price.amount.native.toFixed(4) : "N/A",
...
async function calculateRarity(id) {
  ...
  function Rarity(attr, tier) {
    ...
    if (
      EarthData.hasOwnProperty(attr) &&
      typeof EarthData[attr].TotalAmount !== "undefined"
    ) {
...
async function msgBotAttributes(id) {
  ...
  let resourceStr = resources
    .map((r) => {
      return infoEarth[r.key] ?
        `${infoEarth[r.key]}${msgBotDecor(r.tier)}` :
        "";
    })
    .filter(Boolean)
    .join(",");
  ...
  function checkMsgBotDecor() {
    if (resourceStr == "") {
      return `${msgBotKoda("koda")}\nSediment: ${
        infoEarth.sediment
      }${msgBotDecor("sedimentTier")}\nEnvironment: ${
        infoEarth.environment
      }${msgBotDecor(
        "environmentTier"
      )}\nResource: 🚫\nArtifact: ${msgBotArtifact("artifact")}`;
    }
    return `${msgBotKoda("koda")}\nSediment: ${infoEarth.sediment}${msgBotDecor(
      "sedimentTier"
    )}\nEnvironment: ${infoEarth.environment}${msgBotDecor(
      "environmentTier"
...
async function getFloorPrice() {
  ...
  const response = await fetch(
    "https://api-mainnet.magiceden.dev/v3/rtp/ethereum/tokens/v6?collection=0x34d85c9cdeb23fa97cb08333b511ac86e1c4e258&sortBy=floorAskPrice&limit=1&includeTopBid=false&excludeEOA=false&includeAttributes=True&includeQuantity=false&includeDynamicPricing=false&includeLastSale=false&normalizeRoyalties=false",
    options
  );
  const data = await response.json();
...
async function informationAtAFloorPrice(count) {
  ...
  const response = await fetch(
    `https://api-mainnet.magiceden.dev/v3/rtp/ethereum/tokens/v6?collection=0x34d85c9cdeb23fa97cb08333b511ac86e1c4e258&sortBy=floorAskPrice&limit=10&includeTopBid=false&excludeEOA=false&includeAttributes=True&includeQuantity=false&includeDynamicPricing=false&includeLastSale=false&normalizeRoyalties=false`,
    options
  );
  const data = await response.json();
  ...
  if (msg.text == "/floor") {
    ...
    bot.on("callback_query", async (query) => {
      if (query.data === "attributes") {
        ...
        const backMessage = await bot.sendMessage(
          msg.chat.id,
          `${attributes}`, {
            reply_markup: {
      ...
      if (query.data === "rarity") {
        const rarity = await conclusionRarity(dataInfo);
        if (lastMessageId) {
          await bot.deleteMessage(msg.chat.id, lastMessageId);
        }
        const backMessage = await bot.sendMessage(
...
async function commandSearch(msg) {
  if (msg.text === '/start') {
    const startMessage = await bot.sendMessage(msg.chat.id, 'Привет! Это бот, который поможет тебе найти  землю в Otherside. Какую операцию хочешь выполнить?', {
      reply_markup: {
    ...
    if (msg.text == "Найти землю по ID") {
      ...
      if (currentQueryHandler) {
        ...
        currentQueryHandler = async (msg) => {
          if (msg.text.length === 5 && moreInfo == true && /^\d+$/.test(msg.text)) {
            const id = msg.text;
            let infoEarth = await getInfoEarth(id);
            let Keyboard = deleteKeyboard(id);
            ...
            bot.on("callback_query", async (query) => {
              if (query.data === "attributes") {
                let attributes = await msgBotAttributes(msg.text);
                await deleteLastMessage(msg.chat.id);
                let backMessage = await bot.sendMessage(
                  msg.chat.id,
                  `${attributes}`, {
                    reply_markup: {
                      inline_keyboard: BackKeyboard,
              ...
              if (query.data === "rarity") {
                let rarity = await conclusionRarity(id);
                await deleteLastMessage(msg.chat.id);
                let backMessage = await bot.sendMessage(
                  msg.chat.id,
                  `Редкость: ${rarity}`, {
